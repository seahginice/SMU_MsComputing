---
title: "Comparision between timetk and tidyvert family to perform time series forecasting-Part 4"
description: |
...
Sq_stock %>%
  plot_time_series(date, adjusted, .interactive = TRUE, .plotly_slider=TRUE)
```
...
* Model 4: PROPHET is comparable to the ETS models, but has a slightly wider test error confidence interval.
* Model 5: LM is over-shooting the local trend. As the trend component is a simple linear line, it does not account for the change points.
* Model 6: EARTH is overfitting the local trend. This is due to not tuning the number of change points, so the algorithm is auto-calculating the change points.
